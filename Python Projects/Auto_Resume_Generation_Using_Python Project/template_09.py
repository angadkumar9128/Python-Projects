...
def create_resume(output_path):
    ...
    contact_data = [[Paragraph(format_input(key, value), normal_style)] for key, value in contact_details]
    table_data = [
        [Image(img_path, width=img_width, height=img_height), Spacer(1, 70, 260), Table(contact_data, colWidths=[400])]
    ]
    table = Table(table_data, colWidths=[img_width, 70, 260, 400])
    ...
    while True:
        ...
        edu_details = format_input(degree, f"{university}, {start_date_edu} to {end_date_edu}")
    ...
    # Exprience
    flowables.append(Spacer(1, 12))  # Add space between achievements and experience
    ...
    while True:
        ...
        flowables.append(Paragraph(f"{start_date_exp} to {end_date_exp}", normal_style))
    ...
    while True:
        ...
        achievement_details = f"{start_date_ach} to {end_date_ach}, {achievement_desc}"
        flowables.append(Paragraph(achievement_details, normal_style))
    ...
    while True:
        ...
        project_details = f"{start_date_proj} to {end_date_proj}, {project_desc}"
        flowables.append(Paragraph(project_details, normal_style))
